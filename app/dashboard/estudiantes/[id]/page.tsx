...
export default function FichaEstudiante() {
  ...
  const [nuevoRegistro, setNuevoRegistro] = useState({
    lenguaje: 4, matematica: 4, socioemocional: 4,
    conducta: 4, autonomia: 4, ciencias: 4, observaciones: ""
  });
  ...
  async function agregarRegistro() {
    ...
    if (!user) return;
    const { data } = await supabase.from("registros_aprendizaje").insert({
      ...nuevoRegistro, estudiante_id: id, docente_id: user.id
    }).select().single();
  ...
  const promedios = areas.map(a => {
    const vals = registros.map(r => r[a.key]).filter(v => v > 0);
    return { ...a, promedio: vals.length ? vals.reduce((s, v) => s + v, 0) / vals.length : 0 };
  });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-start flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => setEditando(!editando)}
            className="px-4 py-2 border border-indigo-300 text-indigo-600 rounded-lg text-sm hover:bg-indigo-50 transition">
            ✏️ Editar
          </button>
          <button onClick={eliminar}
            className="px-4 py-2 border border-red-300 text-red-500 rounded-lg text-sm hover:bg-red-50 transition">
            🗑️ Eliminar
          </button>
        </div>
      </div>
      ...
      {editando ? (
        <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
          ...
          <div className="flex gap-3">
            <button onClick={() => setEditando(false)} className="flex-1 border border-gray-300 text-gray-600 py-2 rounded-lg text-sm hover:bg-gray-50">Cancelar</button>
            <button onClick={guardarEdicion} className="flex-1 bg-indigo-600 text-white py-2 rounded-lg text-sm hover:bg-indigo-700">Guardar</button>
          </div>
        </div>
      ) : (
            ...
            {promedios.map(a => (
              <div key={a.key} className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-gray-100 rounded-full h-5 relative">
                  <div className={"h-5 rounded-full transition-all " + a.color}
                    style={{ width: (a.promedio / 7) * 100 + "%" }} />
                </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-6">
            ...
            {debilidades.length > 0 && (
              <div className="bg-red-50 rounded-lg p-4">
                <h3 className="font-semibold text-red-700 text-sm mb-2">⚠️ Areas a mejorar</h3>
      ...
      {registros.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="font-semibold text-gray-700 mb-4">📅 Historial de registros</h2>
          <div className="space-y-3">
            {registros.map(r => (
