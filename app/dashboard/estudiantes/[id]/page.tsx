...
export default function FichaEstudiante() {
  ...
  async function guardarEdicion() {
    ...
    setEstudiante(form); setEditando(false);
  }
  ...
  if (!estudiante) return <div className="text-center py-12 text-gray-400">No encontrado</div>;
  ...
  const resumenPDF = "FICHA: " + estudiante.nombre + "\nCurso: " + (estudiante.curso || "-") + "\nFortalezas: " + fortalezas.map(f => f.label).join(", ") + "\nMejorar: " + debilidades.map(d => d.label).join(", ");
          ...
          <div className="grid grid-cols-2 gap-4">
            {[["nombre","Nombre"],["rut","RUT"],["curso","Curso"],["colegio","Colegio"],["apoderado","Apoderado"],["contacto_apoderado","Contacto"],["diagnostico","Diagnostico"],["profesionales","Profesionales"]].map(([k,l]) => (
              <div key={k}>
                ...
                <input value={form[k] || ""} onChange={e => setForm({...form, [k]: e.target.value})} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
              </div>
            ))}
          </div>
      ...
      {registros.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            {fortalezas.length > 0 && <div className="bg-green-50 rounded-lg p-4"><h3 className="font-semibold text-green-700 text-sm mb-2">Fortalezas</h3>{fortalezas.map(f => <div key={f.key} className="text-sm text-green-600">{f.label}: {f.promedio.toFixed(1)}</div>)}</div>}
            {debilidades.length > 0 && <div className="bg-red-50 rounded-lg p-4"><h3 className="font-semibold text-red-700 text-sm mb-2">Mejorar</h3>{debilidades.map(d => <div key={d.key} className="text-sm text-red-600">{d.label}: {d.promedio.toFixed(1)}</div>)}</div>}
          </div>
        </div>
      )}
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h2 className="font-semibold text-gray-700 mb-4">Agregar Registro</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
          {areas.map(a => (
            <div key={a.key}>
              <label className="block text-sm font-medium text-gray-700 mb-1">{a.label} (1-7)</label>
              <input type="number" min="1" max="7" step="0.1" value={nuevoRegistro[a.key as keyof typeof nuevoRegistro]}
                onChange={e => setNuevoRegistro({...nuevoRegistro, [a.key]: parseFloat(e.target.value)})}
                className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
            </div>
          ))}
        </div>
        <textarea rows={2} value={nuevoRegistro.observaciones} onChange={e => setNuevoRegistro({...nuevoRegistro, observaciones: e.target.value})}
          className="w-full border border-gray-300 rounded-lg px-4 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="Observaciones..." />
        <button onClick={agregarRegistro} disabled={guardandoRegistro} className="w-full bg-indigo-600 text-white py-3 rounded-lg font-semibold hover:bg-indigo-700 disabled:opacity-50">
